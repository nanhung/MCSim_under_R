...
Here are the R functions that can help you run GNU MCSim in R environment more easily. All R functions are put in `functions.R`.
...
- Creating MCSim program `mod.exe`. This process will automatically run when loading `function.R` and doesn't need to run again. 
...
- Using the compiled program with the input file to run the simulation. The `input.file` assignment is a string giving the name of the input file (e.g., `"linear.in.R"`)
